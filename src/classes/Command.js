...
class BaseCommand {
    ...
    get embed() {
        return class extends MessageEmbed {
            constructor(options) {
                ...
                this.footer = {
                    text: "ALPS BTE",
                    iconURL: "https://cdn.discordapp.com/icons/696795397376442440/a_65cf5b07b37f35fb0a69926b4946662f.gif?size=256"
                };
            };
        };
    };
	...
	async initialize(guildId) {
        ...
        guild.commands.create({
            ...
            cmd.permissions.add({ command: cmd.id, permissions: perms }).then(() => {
	...
	async response(interaction, input, components = []) {
        if(typeof input === "object") {
            if(input.description) return await interaction.editReply({ embeds: [ input ], components: components }).catch(this.client.Logger.error);
            else return await interaction.editReply({ embeds: input, components: components }).catch(this.client.Logger.error)
        } else if(typeof input === "string") {
            return await interaction.editReply({ content: input, components: components }).catch(this.client.Logger.error);
        }
	}
	...
	error(interaction, text) {
        return interaction.editReply({ embeds: [new MessageEmbed().setColor('#ff0000').setDescription(":x: " + text)] }).catch(this.client.Logger.error);
	}

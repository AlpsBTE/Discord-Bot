...
class RejectCommand extends Command {
  ...
  async run(interaction: CommandInteraction, client: Bot): Promise<void> {
      ...
      suggUser
        .send({
          embeds: [
            new MessageEmbed()
              .setDescription(
                `You're suggestion (\`${suggestionId}\`) in ${suggestionMessage.guild?.name} was rejected, you can find it [here](${suggestionMessage.url}).`
              )
              .setColor("RED"),
          ],
        })
    ...
    return this.response(
      interaction,
      this.embed.setDescription(`Suggestion ${suggestionId} was rejected.`)
    );
  }
}

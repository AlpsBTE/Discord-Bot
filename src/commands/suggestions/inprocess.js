...
class InprocessCommand extends Command {
    ...
    async run(interaction, client) {
        ...
        newSuggEmbed
            .addField("Status:", `Inprocess (${considerText})\n~ ${interaction.user.tag}`)
            .setColor("ORANGE");
        ...
        if(suggUser) suggUser.send({ embeds: [ new MessageEmbed()
            .setDescription(`You're suggestion (\`${suggestionId}\`) in ${suggestionMessage.guild.name} was set to inprocess, you can find it [here](${suggestionMessage.url}).`)
            .setColor("ORANGE")
        ]}).catch(e => {});

        return this.response(interaction, new this.embed()
            .setDescription(`Suggestion ${suggestionId} was inprocessed.`)
        );
    };
};
